...
在「我」页可选择（对齐双轨：线上静默 / 线下感知）：

| 模式 | 行为 |
|------|------|
| **关闭** | 线上静默：陌陌私聊与近况**不注入**主线 |
| **软互通**（默认） | 线下感知：匹配/加好友近况 + **近期私聊摘要**写入酒馆 **扩展提示槽**（`setExtensionPrompt`），主线生成时可感知；**不写**主聊天气泡，也不把私聊逐条塞进 `chat` |
| **硬注入** | 软互通 + 仅匹配成功/加好友时写一条「剧情同步」系统气泡 |

刷动态**永不**写入主聊天。旧版「线下模式」若曾开启，会迁移为硬注入。若你仍感觉隔离，请确认「我」页互通已设为软/硬并点保存，再在主线触发一次生成。
